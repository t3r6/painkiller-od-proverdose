...
{ 
    ...
    PasswordAlreadyRequested = false,
...
function Game:ApplySettings()
    MOUSE.SetSensitivity(Cfg.MouseSensitivity)
    SOUND.ApplySoundSettings( Cfg.MasterVolume, Cfg.MusicVolume, Cfg.SfxVolume, Cfg.SoundChannelsConfiguration, Cfg.SoundPan )
end
--============================================================================
...
function Game:Tick(delta)        
	if 	Game.DEEG then Game:Print("CP deb-1") end
    if self.WaitForServer then return end

	if 	Game.DEEG then Game:Print("CP deb-2") end
    luaProfiler_LOGIC1()
    DELTA = delta
    MX, MY = MOUSE.GetPos()   
    Game:UpdateTimer(delta)
        
	if 	Game.DEEG then Game:Print("CP deb-3") end
	...
	if 	Game.DEEG then Game:Print("CP deb-4") end
    if Game.GMode ~= GModes.SingleGame then
	...
	if 	Game.DEEG then Game:Print("CP deb3") end
    luaProfiler_LOGIC4a()
	...
	if 	Game.DEEG then Game:Print("CP deb4") end
	...
	if 	Game.DEEG then Game:Print("CP deb5") end
    Hud:Tick(delta)
    --if MENU.Active() then Menu:Tick(delta) end

	if 	Game.DEEG then Game:Print("CP deb6") end
	...
	if 	Game.DEEG then Game:Print("CP deb10") end
    if StringToDo then
        dostring(StringToDo)
        StringToDo = nil
    end

	if 	Game.DEEG then Game:Print("CP deb11") end
    ...
    luaProfiler_LOGIC5()
    OMX, OMY = MX, MY
	if 	Game.DEEG then Game:Print("CP deb16") end
end
...
function Game:Update()
    ...
    -- sound ugly hack
    if Game.BodyCountTotal > 4 then
		if Game.BodyCountTimes[Game.BodyCountTotal] - Game.BodyCountTimes[Game.BodyCountTotal-4] < 3*30 and Game.BodyCountTimes[Game.BodyCountTotal] +40 > Game.currentTime then
			if Game.BodyCountTypes[Game.BodyCountTotal]==2 and Game.BodyCountTypes[Game.BodyCountTotal-1]==2 and Game.BodyCountTypes[Game.BodyCountTotal-2]==2 and Game.BodyCountTypes[Game.BodyCountTotal-3]==2 and Game.BodyCountTypes[Game.BodyCountTotal-4]==2 then
				if Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime then
					Game.lastchat=Game.currentTime
					PlaySound2D("belial/Belial_ingame_"..math.random(120,124),Game.chatvolume,nil,true)
				end
			end
		end
    end
    if Game.BodyCountTotal > 10 then
		if Game.BodyCountTimes[Game.BodyCountTotal] - Game.BodyCountTimes[Game.BodyCountTotal-9] < 15*30 and Game.BodyCountTimes[Game.BodyCountTotal] +40 > Game.currentTime  then
				if Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime then
					Game.lastchat=Game.currentTime
					PlaySound2D("belial/Belial_ingame_"..math.random(152,157),Game.chatvolume,nil,true)
				end
		end
    end
...
function Game:PostRender(delta) -- do HUD'a
	...
	if MPCfg.GameState == GameStates.WarmUp then
		...
		if not Cfg.PROD_SimpleIcons then
			...
			HUD.PrintXY((w-HUD.GetTextWidth("Warm Up"))/2+1,601*h/768,"Warm Up","timesbd",15,15,15,26)
			HUD.PrintXY((w-HUD.GetTextWidth("Warm Up"))/2,600*h/768,"Warm Up","timesbd",230,161,97,26)
		else
			HUD.PrintXY((w-HUD.GetTextWidth("Warm Up"))/2+1,601*h/768,"Warm Up","../ProOptions/Fonts/impact",15,15,15,26)
			HUD.PrintXY((w-HUD.GetTextWidth("Warm Up"))/2,600*h/768,"Warm Up","../ProOptions/Fonts/impact",255,255,255,26)
		end
    end
--MODIFIED=end#######################################################################
...
function Game:OnPlay(firstTime)
    ...
    if Lev.OnPlay then 
        ...
        if Player then
		Player._ambientdelay=0
		if Player.Health < 999 and self.InitialHealth ~= 100 then Player.Health = self.InitialHealth end
	        if self.InitialAmmo ~= 100 then
			Player.Ammo = Clone(CPlayer.s_SubClass._666Ammo)
        	end
		if self.ArmorBonus and Player.Armor and Player.Armor < self.ArmorBonus then
...
--	MagicBoard_LoadStatus()
	...
	if Game.CardsSelected[cards[10].index] then
		self.HealthCapacity = 110
		self.InitialHealth = 110
	end
end
--============================================================================
function Game:ResetGoldenCardsVars()
	...
	self.Thundersteps = false
	self.BelialAssault = false
end
--============================================================================
...
--		self.GoldenEnableTime = self.GoldenEnableTime + 20
	...
	-- Assault misto double haste
	if Game.CardsSelected[cards[9].index] then
		Game.BelialAssault = true
	end
	
	-- steps of thunder misto triple haste 
	if Game.CardsSelected[cards[10].index] then
		Game.Thundersteps = true 
	end
	...
	-- Gamble
	if Game.CardsSelected[cards[14].index] then
		self.DamageFactor = math.random(0.8,2.5)
	end
...
--]]
	...
	crx = crx + mdx
    cry = cry + mdy
--Game:Print("pozice="..destPos.X..","..destPos.Y..","..destPos.Z)
...
--    SOUND.ApplySoundSettings( Cfg.MasterVolume, 100, Cfg.SfxVolume, Cfg.SoundChannelsConfiguration, Cfg.SoundPan, Cfg.ReverseStereo )
