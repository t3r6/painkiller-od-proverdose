...
function Cache:PrecacheActor(objname,scale)
    ...
    if obj.enableGibWhenHPBelow  and obj.Model ~= "bones" and (not IsGermanBuild() or obj.Model== "tank" or obj.Model == "tank_FA" or obj.Model == "tiger" or obj.Model == "t34") then
...
function Cache:PrecacheItem(objname)
    ...
    if not Cached[hash] then 
        ...
        if fLog then
            fLog:write(obj.Pack..","..obj.Mesh..","..obj.Scale.."\n")
        end
...
function Cache:PrecacheLevel(name)
    ...
    self:PrecacheActor("RazorCube.CWeapon")       
    self:PrecacheActor("HellBlade.CWeapon")        
    self:PrecacheActor("Ectoplasmer.CWeapon")        
    self:PrecacheActor("Screamer.CWeapon")        
    self:PrecacheActor("Bonegun.CWeapon")        
    self:PrecacheActor("CannonGun.CWeapon")        
    self:PrecacheActor("EggBomb.CWeapon")        
    self:PrecacheActor("MagicCrossbow.CWeapon")        
