...
function SaveGame:SaveOtherData(slot,demo)
	local path
	if not demo then
		path = string.format(GetSaveDirectory().."%03d/",slot)
	else
		path = GetRecordingsDirectory()..""..slot.."/"
	end
...
function SaveGame:Load(slot,loadOnly,demo)
    ...
    local pack = FS.RegisterPack(path.."Save.OverdoseSave",path)
...
function SaveGame:FindFreeSlot()
    ...
    local slot = 0
    local files = FS.FindFiles(GetSaveDirectory().."*.*",0,1)
...
function SaveGame:SaveRequest(slot,stype,demo)
    ...
    Game:Print("SAVINGREQUEST")
end
--============================================================================
function SaveGame:Save(slot,stype,forcetime,demo)
Game:Print("STARTING SAVE")
    SaveRequest = nil
...
--		FS.CreateDirectory("../Recordings")
    ...
    FS.DeleteFiles(path)
    	Game:Print("STARTING SAVE AFTER CREATE")
    SaveGame.Type = stype
    ...
    if stype == "NewLevel" or stype == "AutoNewLevel" or stype == "StartLevel" then 
		...
		if Game.CurrChapter > 0 and Game.CurrLevel > 0 then
			Game:Print("cc a cl"..Game.CurrChapter)
...
Game:Print("SAVE AFTER FULL OBJ")
    for i,o in GObjects.Elements do 
Game:Print("SAVE OBJ.."..o._Name)
        SaveFullObj(path..o._Name.."."..o._Class,o)            
    end        
Game:Print("SAVE AFTER ELEMENTS OBJ")
    self:SaveOtherData(slot,demo)
Game:Print("SAVE OTHERDATA")
    WORLD.SaveGame(path..Lev._Name..".World")
Game:Print("SAVE AFTER WORLDSAVE")
