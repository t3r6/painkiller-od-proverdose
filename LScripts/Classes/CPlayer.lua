...
CPlayer = 
{
    ...
    EnabledWeapons = {"RazorCube","Bonegun","CannonGun","MagicCrossbow","Screamer","HellBlade","Ectoplasmer","EggBomb"},
...
_koliks={},
    ...
    sndBelialHit = {"impacts/bullet-body","impacts/bullet-body2",},
...
function CPlayer:SndEnt(id,entity)
    if Game.GMode == GModes.SingleGame then
	    if Game.GHero == GModes.Daniel then
	        CObject.SndEnt(self,id,entity)
	     else
		     self.s_SubClass.NSoundDefinitions =  self.s_SubClass.SoundDefinitions 
     		     self.s_SubClass.SoundDefinitions =  self.s_SubClass.AltSoundDefinitions 
       	        CObject.SndEnt(self,id,entity)
		     self.s_SubClass.SoundDefinitions =  self.s_SubClass.NSoundDefinitions 
	end
    else
...
function CPlayer:ServerTick(delta)
	...
	self.Ammo.Ectoplasm   =  999
	self.Ammo.GreenGoo	 = 999
	self.Ammo.DarkEnergy	 = 999
	self.Ammo.BrainWorms	 = 999
	self.Ammo.BrokenSouls	 = 999
	self.Ammo.StoneBullets	 = 999
	self.Ammo.Bonegun	 = 999
	self.Ammo.Skulls	 = 999
	self.Ammo.Cannonball	 = 999
	self.Ammo.Arrows	 = 999
	self.Ammo.Heads	 = 999
	self.Ammo.StickyBombs	 = 999
    end
...
function CPlayer:InterpretAction(delta)
    ...
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.Weapon8) then
        self:TryToChangeWeapon(8)
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.Weapon9) then
	    Game:Print("STATE 9")
        self:TryToChangeWeapon(9)
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.Weapon10) then
	    Game:Print("STATE 10")
        self:TryToChangeWeapon(10)
    elseif ENTITY.PO_IsActionState(self._Entity,Actions.Weapon11) then
	    Game:Print("STATE 11")
        self:TryToChangeWeapon(11)
    end
    ...
    if cw then                    
        ...
        if cw.ShotTimeOut <= 0 then
            if not self._frjump and (ENTITY.PO_IsActionState(self._Entity,Actions.ForwardRocketJump) or ENTITY.PO_IsActionState(self._Entity,Actions.RocketJump))
               and cw.BaseObj == "CannonGun.CWeapon" and Lev._Name ~= "C8L07_Black_tower" then 
                -- physics jump
                --Game:Print("1")
                self._frjump = 0
            end
        ...
        if self._frjump and self._frjump == 1 then
            -- rocket
            if ENTITY.PO_IsActionState(self._Entity,Actions.ForwardRocketJump) and Lev._Name ~= "C8L07_Black_tower" then
...
function CPlayer:Tick(delta)
    ...
    if Game.GMode == GModes.SingleGame then     
		...
		Game.lastchat=Game.currentTime
	        PlaySound2D("belial/Belial_ingame_"..math.random(22,26),Game.chatvolume,nil,true)
		end
    ...
    if not self._died then
        ...
        if Game.GMode == GModes.SingleGame then        
            ...
            -- Find first available weapon.
                ...
                local mx = 1
                for i,o in self.EnabledWeapons do
                    mx = i
                    break
                end
		...
		Game:Print("selecting weapon:"..mx)
            end
...
function CPlayer:OnDamage(damage,killer,attack_type,x,y,z,nx,ny,nz)
    --if killer then Game:Print("KILLER="..killer) end
   ...
   --ugly last time Geisha hack
if Game.GMode == GModes.SingleGame and Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime and Lev and Lev._geishadamage then
			Game.lastchat=Game.currentTime
			 Lev._geishadamage = false
		  	PlaySound2D("belial/Belial_ingame_"..math.random(47,48),Game.chatvolume,nil,true)
	end
...
function CPlayer:ResetStatus(weapon)    
    ...
    self.EnabledWeapons = {"RazorCube","Bonegun","CannonGun","MagicCrossbow",nil,"HellBlade","Ectoplasmer","EggBomb"}
    elseif MPCfg.GameMode == "People Can Fly" then    
        self.Ammo = Clone(CPlayer.s_SubClass.Ammo)
        self.EnabledWeapons = {nil,nil,"CannonGun",nil,nil,nil,nil,nil}    
        local a = Templates["ArmorStrong.CItem"]
        self.Armor             = a.ArmorAdd
        self.ArmorType         = a.ArmorType
        self.ArmorRescueFactor = a.RescueFactor
    else
        self.EnabledWeapons = {"RazorCube","Bonegun"}
        self:AddWeapon(1)
        self:AddWeapon(2)
    end
    ...
    self.EnabledWeapons = {"RazorCube","Bonegun","CannonGun","MagicCrossbow","Screamer","HellBlade","Ectoplasmer","EggBomb"}
        self:AddWeapon(1)
        self:AddWeapon(2)
        self:AddWeapon(3)
        self:AddWeapon(4)
...
function CPlayer:Steps(delta)
    ...
    if self._died then return end
    self._Walking = false        
    self._ambientdelay=self._ambientdelay+1
        ...
        if Game.GMode == GModes.SingleGame then            
            if self:IsOnGround() or ENTITY.PO_IsOnLadder(self._Entity) then
                ...
                if ae.Player.Spurs then
                    PlaySound2D("MOVEMENT/leather/leather"..math.random(1,4))
                end
...
function CPlayer:OnHitGround(speed)
    ...
    if speed < -self.collisionMinSpeed then 
        ...
        if self._GroundSndDelay <= 0 and speed < minspd then
        	if Game.GMode == GModes.SingleGame then
                	PlaySound2D("belial/Belial_ingame_"..math.random(28,32),nil,nil,true)
		else
...
function CPlayer:CheckMaxAmmo()
    ...
    if self.Ammo.DarkEnergy   and self.Ammo.DarkEnergy   > ma.DarkEnergy   then self.Ammo.DarkEnergy   = ma.DarkEnergy   end
    if self.Ammo.GreenGoo   and self.Ammo.GreenGoo   > ma.GreenGoo   then self.Ammo.GreenGoo   = ma.GreenGoo   end
    if self.Ammo.BrokenSouls   and self.Ammo.BrokenSouls   > ma.BrokenSouls   then self.Ammo.BrokenSouls   = ma.BrokenSouls   end
    if self.Ammo.BrainWorms   and self.Ammo.BrainWorms   > ma.BrainWorms   then self.Ammo.BrainWorms   = ma.BrainWorms   end
    if self.Ammo.Arrows   and self.Ammo.Arrows   > ma.Arrows   then self.Ammo.Arrows   = ma.Arrows   end
    if self.Ammo.Heads   and self.Ammo.Heads   > ma.Heads   then self.Ammo.Heads   = ma.Heads   end
    if self.Ammo.Skulls   and self.Ammo.Skulls   > ma.Skulls   then self.Ammo.Skulls   = ma.Skulls   end
    if self.Ammo.Bonegun   and self.Ammo.Bonegun   > ma.Bonegun   then self.Ammo.Bonegun   = ma.Bonegun   end
    if self.Ammo.StoneBullets   and self.Ammo.StoneBullets   > ma.StoneBullets   then self.Ammo.StoneBullets   = ma.StoneBullets   end
    if self.Ammo.Cannonball   and self.Ammo.Cannonball   > ma.Cannonball   then self.Ammo.Cannonball   = ma.Cannonball   end
    if self.Ammo.Ectoplasm   and self.Ammo.Ectoplasm   > ma.Ectoplasm   then self.Ammo.Ectoplasm   = ma.Ectoplasm   end
    if self.Ammo.StickyBombs   and self.Ammo.StickyBombs   > ma.StickyBombs   then self.Ammo.StickyBombs   = ma.StickyBombs   end
    ...
    if player then 
        ...
        if player == Player then
            ...
            if Game.GMode == GModes.SingleGame and (not player._oldSND or not SOUND2D.IsPlaying(player._oldSND)) then
			    ...
			    local snd
	                        snd = player.sndBelialHit[math.random(1,2)]
                        PlaySound2D(snd,nil,nil,true)
                        player._lastTimePlayedSoundHit = Game.currentTime
                    end
				end

                player._oldSND = PlaySound2D("belial/Belial_ingame_"..math.random(17,21),nil,nil,true)
            end
        end
    end
--ADDED=##########################################################################################
	if Player and Player.ClientID == killerID and (not player or player ~= Player) and Cfg.PROD_HitSound then
		PlaySound2D("../PRODsounds/hitsound",nil,nil,true)
	end
--ADDED=end########################################################################################	
...
function CPlayer:UpdateForSoulReaper(team)
	--[[
if MPCfg.GameMode ~= "Team Deathmatch" then return end
	if team == 0 then
        	Game._team1Score = Game._team1Score + score
	        fscore = Game._team1Score
	 else
	 ...
	 if Game:IsServer() and fscore >= MPCfg.FragLimit then
            Game.SetGameState(GameStates.Finished)                
             StringToDo = "Game.EndOfMatch()"
         end	 
--[[	 if ks then
...
Network:RegisterMethod("CPlayer.UpdateForSoulReaper", NCallOn.ServerAndAllClients, NMode.Reliable, "u") 
...
--ADDED=end########################################################################################
    ...
    if Game.GMode == GModes.SingleGame then            
        PlaySound2D("belial/Belial_ingame_"..math.random(9,13),nil,nil,true)
    else
        if gib == 1 then 
