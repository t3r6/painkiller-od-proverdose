...
--============================================================================
function CAction:Action_RPFX(effect,scale,x,y,z,RotObj)
    local v
    if x then 
        v = Vector:New(x,y,z) 
    else
    ...
    local e
    if RotObj then
	local r = Quaternion:New_FromEntity(RotObj._Entity)
    	local e = AddPFX(effect,scale,v,r)    
    else
    	local e = AddPFX(effect,scale,v)    
    end
    return e
...
--============================================================================
function CAction:Action_FAnim(obj,anim,loop,blend,speed)
    if not obj then return end
    obj.Animation = ""
    obj:SetAnim(anim,loop,speed,blend) 
end
--============================================================================
...
--============================================================================
function CAction:Action_IVisible(entity,state)
    if not entity then return end
    if state == nil  then state = true end    
...
--============================================================================
function CAction:Action_GenericParticle(obj,particle,scale)
    if not obj then return end
    local p = Templates["PParticledItem.CProcess"]:New(obj,particle,scale) 
    p:Init()
    GObjects:Add(TempObjName(),p)
end
--============================================================================
...
--============================================================================
function CAction:Action_Vytah(object,speed,accelTime)
    if not Game._loonyProc then
        local p = Templates["Vytah.CProcess"]:New(object,speed,accelTime)
        GObjects:Add(TempObjName(),p)
        Game._loonyProc = p
    else
        if not accelTime and speed == 0 then
...
function CAction:Action_SPPFX(x,y,z,pfx,scale,vx,vy,vz)
    local e = AddParticleFX(x,y,z,pfx)    
...
--============================================================================
function CAction:Action_CannonFX(x,y,z,fxscale,nx,ny,nz)
    if not fxscale then fxscale = 1 end
    -- sound
    SOUND.Play3D("weapons/machinegun/rocket_hit",x,y,z,30,200)
...
--
--
function CAction:Action_TeleportTo(box)
  local ox,oy,oz = ENTITY.GetPosition(Player._Entity)
    local player = EntityToObject[Player._Entity]
    ...
    if player then        
	 local area = getfenv()[box._Name]
        if area then 
       		x,y,z,a = area:GetPoint(1)                 
        	if Game:IsServer() then player:CheckTeleFrag(x,y,z) end
	        ENTITY.SetPosition(Player._Entity,x,y,z)    
	        ENTITY.SetOrientation(Player._Entity,a)
	        local vx,vy,vz,vl = ENTITY.GetVelocity(Player._Entity)
	       vx = math.sin(a)
		vz = math.cos(a)        
       		ENTITY.SetVelocity(Player._Entity,vl*vx,vy,vz*vl)
        end
...
function CAction:Action_StartBugging(template)
	if not template then template = "MutaNemoP.CActor" end
	 local player = EntityToObject[Player._Entity]
	 player.bugged = true
	 local monstern = 4
	 while monstern > 0 do
		...
		local finding = true
		while finding do
			px= x + math.random(-3,3) - Player.ForwardVector.X * 2  
	 		py =y 
			pz= z + math.random(-3,3) - Player.ForwardVector.Z * 2
...
function CAction:Action_BugPlayer(template)
	if not template then template = "MutaNemoP.CActor" end
	 local player = EntityToObject[Player._Entity]
	 if player and not player.bugged then return end
	 local x,y,z = ENTITY.GetPosition(Player._Entity)
		local finding = true
		while finding do
			px= x + math.random(-4,4)
	 		py =y 
			pz= z + math.random(-4,4)
			local zn,idx = WPT.GetClosest(px,py,pz)   		
		    	if idx > -1 then
			        local x,y,z = WPT.GetPosition(zn,idx)    
					finding = false
...
function CAction:Action_StopBugging()
	 local player = EntityToObject[Player._Entity]
	 player.bugged= false
end

function CAction:Action_SwOn(what)
local e = WORLD.FindEntityByName(what)        
        ENTITY.EnableDraw(e,true)
end

function CAction:Action_SwOff(what)
local e = WORLD.FindEntityByName(what)        
        ENTITY.EnableDraw(e,false)
end

function CAction:Action_StartCIN(area,speed)
local o = FindObj(area)
	Game._demoproc =  AddObject(Templates["PCinematic.CProcess"]:New(o,speed))

end

function CAction:Action_meteorspawn(point)
	 pos = Vector:New(point.Pos.X,point.Pos.Y+50,point.Pos.Z) 
	-- local obj = AddObject("Spawnlava.CItem",nil,pos,nil,true)
	 local obj = AddObject("Spawnlava.CItem",nil,pos,nil,true)
	 obj.point=point
	 obj:Apply()
end
...
function CAction:Action_randomunfreeze(first,second,third)
	local a = math.random(3)
	local obj
	obj=first
	if a> 0 then obj=second
		if a>1 then
			obj=third
		end
	end	
...
function CAction:Action_LMOUSE()
 Hud.Enabled = false
 R3D.SetCameraFOV(90)
 ENTITY.PO_Enable(Player._Entity,false) 	
 camera=AddObject(Templates["GCameraControler.CProcess"]:New(guardcam,0.15,"Guardian_001"))
 camera._hidew = true
end

function CAction:Action_Dfade()
AddObject(Templates["FadeInOut.CProcess"]:New(true,6))
end

function CAction:Action_IGS(r1,r2)
	...
	if Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime then
		...
		if not r2 then
			PlaySound2D("belial/Belial_ingame_"..r1,Game.chatvolume,nil,true)
		else
			PlaySound2D("belial/Belial_ingame_"..math.random(r1,r2),Game.chatvolume,nil,true)
		end
	end
end
