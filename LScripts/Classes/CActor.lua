...
function CActor:damage(par3, par4, damageSound,secondr)
	local br = self._AIBrain
	if br and br.r_closestEnemy then
		...
		if secondr then
			weaponRange = aiParams.secondattackRange
		...
		if dist <= weaponRange and math.abs(aDist) < angleRange*math.pi/180 then
			...
			if br.r_closestEnemy.OnDamage then
				...
				if secondr then
				br.r_closestEnemy:OnDamage(aiParams.weapon2Damage,self,AttackTypes.AIClose,x,y,z,nx,ny,nz)
				else
...
if debugAnim then 
Game:Print(self._Name.." >Set anim: "..anim.." "..", blend "..blend..", old anim "..self.Animation.." "..self._LastAnimTime) 
end
...
function CActor:CreateGib(newModel)
    if (Tweak.GlobalData.DisableGibs and not self.notBleeding) or IsGermanBuild() then
    	if not (self.Model == "zombieapo_v2" and self.Health > 0) then
			return false
		end
    end
	if self.GibAnim then
	...
	if gib then
		...
		if self.s_SubClass.gibShader then
			if self._frozen or self._slowed then
				if self._fstoned then
					MDL.SetMaterial(gib, "palskinned_stone")
				else
					MDL.SetMaterial(gib, "palskinned_freeze")
				end
			elseif self._slowed2 then
				MDL.SetMaterial(gib, "palskinned_greened")
			else
...
function CActor:PO_Create(bodytype,bodyscale,collisionGroup)
    if not ENTITY.PO_Exist(self._Entity) then
		...
		if phys.Collgroup then
			collisionGroup = phys.Collgroup
		end
            else
...
function CActor:RotateTo(ang)
	if not self._rotatingWithAnim and not self.DisableRotate then
...
--Game:Print("seting range orientation="..self.angle)
    ENTITY.SetOrientation(self._Entity,self.angle)
end
...
function CActor:OnDamage(damage, obj, type, x, y, z, nx, ny, nz, he,msg)			-- +pos
	...
	--===============================================================================================
		local gibdie = false
		local egi = self.enableGibWhenHPBelow
		if egi and self._enableGibWhenHPBonusHP then
			egi = egi + self._enableGibWhenHPBonusHP
		end
		if egi and (egi >= self.Health - damage or self._frozen or Game.Cheat_AlwaysGib) then 
			gibdie = true 
		end 

		if self.Health - damage <= 0 and self.DeadAnimChance and math.random(100)< self.DeadAnimChance and not gibdie  then
			self._CustomDeathAnim = true
			self.deathtype = "deathanim"
			...
			if not self.s_SubClass.Animations.death1 then
				self.s_SubClass.Animations.death1   = {1.0,false}
			end
			if not self.s_SubClass.Animations.death2 then
				self.s_SubClass.Animations.death2   = {1.0,false}
			end
			self.AiParams.aiGoals = {"die"}
			self.DeathTimer =1
			self._disableDeathSounds = true
			self.dying = true
			--self.s_SubClass.GibExplosionStrength = 0
			self.s_SubClass.Deaths={"death2","death1"}
			--self.enableGibWhenHPBelow = nil
			local brain = self._AIBrain
			if brain then
				local aiParams = self.AiParams
				for i,v in aiParams.aiGoals do
		...
		if self.Health <= 0 then
			if self.IsBoss then
				if Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime then
					...
					PlaySound2D("belial/Belial_ingame_"..math.random(145,151),Game.chatvolume,nil,true)
				end
			end
			if type and type==AttackTypes.Painkiller and  Game.GMode == GModes.SingleGame and Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime and Player and (Player.state == 11 or Player.state == 61 )then
				Game.lastchat=Game.currentTime
				PlaySound2D("belial/Belial_ingame_"..math.random(140,142),Game.chatvolume,nil,true)
			end
			...
			self._sndHitId = nil
				...
				Game.BodyCountTimes[Game.BodyCountTotal] = Game.currentTime
				Game.BodyCountTypes[Game.BodyCountTotal] = 1
				if gibdie then
					Game.BodyCountTypes[Game.BodyCountTotal] = 2
				end
				if Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime and math.random(10) < 5 then
					Game.lastchat=Game.currentTime
					PlaySound2D("belial/Belial_ingame_"..math.random(124,130),Game.chatvolume,nil,true)
				end
			...
			if not self._ToKill and not self.dying then
				self:Stop()
				self:EnableRagdoll(true,true)
				if self.s_SubClass.SoundsDefinitions.death and self.deathtype=="normal" and not self._disableDeathSounds then
					self:BindRandomSound("death",nil,nil, self:GetAnyJoint())
				end
			end
			if type == AttackTypes.Electro or  type == AttackTypes.Scream then
					self:Electrize(type)
			end
			self.Health = 0
			...
			if not cantDo and not self.dying then
...
function CActor:Electrize(mytype)
...
function CActor:ThrowHeart()
	...
	if self.throwHeart then
		...
		if self.deathtype ~= "normal" then
			 obj.Pos:Set(self._groundx,self._groundy + 1, self._groundz)
		else
	...
	if not self._gibbed and self._ragdollShockTime then
		...
		if self._ragdollShockTime > 0 then
			if self.s_SubClass.DeathJoints then
				...
				--]]
				local t2
				if self._electrotype == AttackTypes.Scream then
					t2 = Templates["Screamer.CWeapon"].s_SubClass
				else
					t2 = Templates["DriverElectro.CWeapon"].s_SubClass
				end
...
function CActor:BloodFX(x,y,z,nx,ny,nz)
    --dust
	if self._DustBlood then
		if nx then
				self:AddPFX("FootFX",0.3,Vector:New(x,y,z),Quaternion:New_FromNormal(nx,ny,nz))
		end
	else
    -- blood
		...
		for i = 1, n do
				--greenblood
			if self._GreenBlood or IsGermanBuild() then
		        	local ke = AddItem("GBlood.CItem",0.1,Vector:New(x,y,z),true)
			else
				if self._PurpleBlood then
		        		local ke = AddItem("PBlood.CItem",0.1,Vector:New(x,y,z),true)
				else
		    ...
		    if nx then
		        if Tweak.GlobalData.GermanVersion then
					self:AddPFX("BodyBlood_green",0.3,Vector:New(x,y,z),Quaternion:New_FromNormal(nx,ny,nz))
				...
				if self._GreenBlood then
					...
					if self._PurpleBlood then
						self:AddPFX("BodyBlood_purple",0.3,Vector:New(x,y,z),Quaternion:New_FromNormal(nx,ny,nz))
					else
					self:AddPFX("bloodnew_splat3",0.5,Vector:New(x,y,z),Quaternion:New_FromNormal(nx,ny,nz))
					end
				end
...
--============================================================================
function CActor:SlowObject(obj,SlowMod)
        if not obj._slowed2 then 
		obj._slowed2 = true
		obj._speeded = true
		obj.SlowMod=SlowMod
		AddObject(Templates["SlowedObject.CProcess"]:New(self),nil,nil,nil,true) 
		if self.CustomOnSlow then
			self:CustomOnSlow()
			return
		end	
    	end 
...
--============================================================================
function CActor:SlowObject2(obj,SlowMod)
        if not obj._slowed then 
		obj._slowed = true
		obj._speeded = true
		obj.SlowMod=SlowMod
		AddObject(Templates["SlowedObject2.CProcess"]:New(self),nil,nil,nil,true) 
		if self.CustomOnSlow then
			self:CustomOnSlow()
			return
		end	
...
function CActor:FleshToStone()
    if self._frozen then return end
    if self.disableFreeze then return end
    local s = Templates["Bonegun.CWeapon"]:GetSubClass()
    local ftime = s.FrozenTime
    if Player.HasWeaponModifier then ftime = ftime * 2 end
    AddObject(Templates["FleshToStone.CProcess"]:New(self,ftime,s.FrozenFadeInTime,s.FrozenFadeOutTime),nil,nil,nil,true) 
    if self.CustomOnFreeze then
		self:CustomOnFreeze()
		return
    end
