...
function CWeapon:InterpretAction()    
    ...
    -- special fire (fast combo - alt and fire without timeout) -- disabled
...
function CWeapon:Tick(delta)    
    ...
    if self.ShotTimeOut < -1 and self._ActionState == "Idle" then
		...
		if self._weaponwalk then
	            self:SetAnim(self.weaponwalk) 
		else
	            self:SetAnim("walk") 
	    	end
        else
		if self._weaponwalk then
	            self:SetAnim(self.weaponidle) 
		else
			if self.SecondaryIdle then
				if not self._Idledelay then self._Idledelay = self.Idledelay 
				self._isanim = "idle"
				
				end
				self._Idledelay = self._Idledelay -1 
				
				if self._Idledelay < 1 and self.Animation == "idle" then
					self._isanim = "idle1"
					self._Idledelay = self.Idledelay
				end
				if self._Idledelay < 1 and self.Animation == "idle1" then
					self._isanim = "idle"
					self._Idledelay = self.Idledelay*4
				end
	        		self:SetAnim(self._isanim) 
			else
				if self.ObjOwner._JumpSndDelay and self.ObjOwner._JumpSndDelay > 0 and self.Model == "razorcube" then
				            self:SetAnim("idle2") 
				else
			        	self:SetAnim("idle") 
				end
