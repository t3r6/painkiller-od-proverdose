...
function CAiBrain:OnUpdate()
	...
	--]]
	if actor.ingroup then
		if math.random(300) < 5 and not self._confuseEnemy then
		Game:Print("enemytest PASSED")
			for i,v in Actors do
				if v.Health > 0 and v.ingroup and v.ingroup ~= actor.ingroup then
					self._confuseEnemyTimer = 30
					self._confuseEnemy = v
					self._forceUpdateVisibility = true
					self._enemyLastSeenPoint.X = v._groundx
					self._enemyLastSeenPoint.Y = v._groundy
					self._enemyLastSeenPoint.Z = v._groundz
					actor:Stop()
				end
			end
		end
