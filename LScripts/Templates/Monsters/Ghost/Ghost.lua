...
AiStates.ghostfly = {			-- narazie, pozniej wywalenie idle
	name = "ghostfly",
}

function AiStates.ghostfly:OnUpdate(brain)
local actor = brain._Objactor
if actor.Destpos and actor.Destpos.X then 
	if Dist3D(actor.Destpos.X,actor.Destpos.Y,actor.Destpos.Z,actor.Pos.X,actor.Pos.Y,actor.Pos.Z) <1 then
		GObjects:ToKill(actor)
	else
		actor:FlyTo(actor.Destpos.X,actor.Destpos.Y,actor.Destpos.Z)
	end
end
end

function AiStates.ghostfly:Evaluate(brain)
	return 0.9
end
