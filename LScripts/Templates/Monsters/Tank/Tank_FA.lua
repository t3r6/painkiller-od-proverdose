...
function Tank_FA:OnCreateEntity()
	...
	self._engineSnd = self:BindSound("actor/tank_FA/tank_motor",150,100,true)
	self._moveSnd = nil
	self._turretSnd = nil
	...
	self._Flamebox=PlayerFlameBox_001
...
function Tank_FA:CustomOnDeathAfterRagdoll()
			Game.MegaBossHealth = nil
			Game.MegaBossHealthMax = nil
			Game.TankFA=false

	PlaySound2D("actor/gibs/militarybasegun_gib")
	
		if self._Flamebox then
		GObjects:ToKill(self._Flamebox)
		end
		Game._EarthQuakeProc:Add(self.Pos.X,self.Pos.Y,self.Pos.Z, 55, 65, 2 , 2, 1.0)
		local j = MDL.GetJointIndex(self._Entity, "chinmey")
		local x,y,z = MDL.TransformPointByJoint(self._Entity, j,0,0,0)
	       AddPFX("Grenade",4.0,Vector:New(x,y,z))
	       AddPFX("FAtanksmoke",5,Vector:New(x,y,z))
		j = MDL.GetJointIndex(self._Entity, "l_wheel")
		x,y,z = MDL.TransformPointByJoint(self._Entity, j,0,0,0)
	       AddPFX("Grenade",3.5,Vector:New(x,y,z))
		AddPFX("barrelarabxplo",3.0,Vector:New(x,y,z))
		AddPFX("flame_shadow",1.3,Vector:New(x,y,z))
		j = MDL.GetJointIndex(self._Entity, "r_wheel")
		x,y,z = MDL.TransformPointByJoint(self._Entity, j,0,0,0)
	       AddPFX("Grenade",3.5,Vector:New(x,y,z))
		AddPFX("barrelarabxplo",3.0,Vector:New(x,y,z))
		AddPFX("flame_anim",1.1,Vector:New(x,y,z))
end

function Tank_FA:RotateToVector(tx,ty,tz)
	self._angleDest = math.atan2(tx - self._groundx, tz - self._groundz)
	self:RotTank()
end

function Tank_FA:RotTank()
...
function Tank_FA:OnTick(delta)
	...
	if math.abs(self._AIBrain._diffInangleAttackX) > 0.01 then
		if not self._turretSnd then
			self._turretSound, self._turretSnd = self:BindSound("actor/tank_FA/tank_head",20,100, true)
	...
	if self._AIBrain.r_closestEnemy and Player and not self._disableRotateHead then
		local joint = MDL.GetJointIndex(self._Entity, self.AiParams.weaponRotPos)
...
--		Game:Print("rothead.."..self._barrelPitch)
		MDL.ApplyJointRotation(self._Entity, joint,0, self._barrelPitch,  0)
...
Tank_FA._CustomAiStates = {}
Tank_FA._CustomAiStates.tankIdle = {
	name = "tankIdle",
}

function Tank_FA._CustomAiStates.tankIdle:OnInit(brain)
...
function Tank_FA._CustomAiStates.tankIdle:OnUpdate(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
...
function Tank_FA._CustomAiStates.tankIdle:OnRelease(brain)
	local actor = brain._Objactor
end


function Tank_FA._CustomAiStates.tankIdle:Evaluate(brain)
	return 0.01
end


---------------------------------
Tank_FA._CustomAiStates.tankFire = {
	name = "tankFire",
	delayRandom = FRand(0,1),
}

function Tank_FA._CustomAiStates.tankFire:OnInit(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
...
function Tank_FA._CustomAiStates.tankFire:OnUpdate(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
	if self.delay then
		...
		if self.delay == 0 then
			local e
			local idx  = MDL.GetJointIndex(actor._Entity, aiParams.weaponBindPos1)
			...
			local pd = actor.s_SubClass.ParticlesDefinitions.shot1
			...
			local idx  = MDL.GetJointIndex(actor._Entity, aiParams.weaponBindPos2)
			local q = Quaternion:New_FromEuler(0, math.pi/2 - actor._angleAttackX - actor.angle, math.pi/2)
			local x,y,z = MDL.TransformPointByJoint(actor._Entity, idx, aiParams.weaponBindPosShift.X, aiParams.weaponBindPosShift.Y, aiParams.weaponBindPosShift.Z)
			e, actor._objTakenToThrow = AddItem(aiParams.ThrowableItem, nil, Vector:New(x,y,z),true,q)
			actor._objTakenToThrow.ObjOwner = actor
			local pd = actor.s_SubClass.ParticlesDefinitions.shot2
			AddPFX(pd.pfx, pd.scale, Vector:New(x,y,z), q)
			brain._lockedAngle = actor._angleAttackX
			actor._disableRotateHead = true
			ENTITY.PO_Enable(e,false)			
...
function Tank_FA._CustomAiStates.tankFire:OnRelease(brain)
	local actor = brain._Objactor
	local aiParams = actor.AiParams
...
function Tank_FA._CustomAiStates.tankFire:Evaluate(brain)
...
function o:CustomOnDamage(he,x,y,z,obj, damage, type,nx,ny,nz)
	if type == AttackTypes.Demon or type==AttackTypes.Electro then
	        return true 
	    end
	return false
end
