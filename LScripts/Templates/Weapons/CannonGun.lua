o._ShotInterval = 0
--============================================================================
function CannonGun:OnReleaseEntity()
    if self._sndRotor then
        SOUND2D.Delete(self._sndRotor)
        SOUND2D.Delete(self._sndRotor2)
    end
    ENTITY.Release(self._smokepfx1)
    ENTITY.Release(self._smokepfx2)
    ENTITY.Release(self._smokepfx3)
end
--============================================================================
function CannonGun:OnCreateEntity()
    ...
    self:ReloadTextures()
    self._lastfirestart = 0
end
--============================================================================
function CannonGun:OnPrecache()
    CloneTemplate("CannonGun.CWeapon"):LoadHUDData()
	...
	Cache:PrecacheItem("Cannonball_r.CItem")
	Cache:PrecacheDecal('bullethole')
--	Cache:PrecacheDecal('splash')
end
--============================================================================
function CannonGun:ReloadTextures()
...
--============================================================================
function CannonGun:LoadHUDData()
...
--	self._matAmmoIcon = MATERIAL.Create("HUD/ammo", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
	...
	self._matAmmoRocketIcon = MATERIAL.Create("HUD/kulomet_koule", TextureFlags.NoLOD + TextureFlags.NoMipMaps)
end
--============================================================================
function CannonGun:MuzzleFlashFX(mode,size)
    ...
    if not self._LightName or getfenv()[self._LightName] == nil then
        ...
        self:MuzzleFlash("cylinder_up_joint",0,0,0.85,size)
    end
end
--============================================================================
function CannonGun:OnChangeWeapon()
...
--============================================================================
function CannonGun:DrawHUD(delta)
    ...
    if not (INP.IsFireSwitched() or (not Game.SwitchFire[4] and Cfg.SwitchFire[4]) or (not Cfg.SwitchFire[4] and Game.SwitchFire[4])) then
		...
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*16)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.Cannonball),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.Cannonball)
		...
		Hud:DrawDigitsText((1024-118*Cfg.HUDSize)*w/1024,((768+Cfg.HUDSize*50)-Cfg.HUDSize*sizey)*h/768,string.sub(string.format("%03d",Player.Ammo.Cannonball),-3),0.9 * Cfg.HUDSize,Player.s_SubClass.AmmoWarning.Cannonball)
	end
end
...
--============================================================================
function CannonGun:AltFire() -- bullets
    if self.ObjOwner.Ammo.MiniGun > 0 then
	 ...
	 if Game.GMode == GModes.SingleGame then
		 self._lastfirestart = Game.currentTime
	end
    else
...
--============================================================================
function CannonGun:StartAltFireFX(pe,ammo)
    local player = EntityToObject[pe]
    if not player then return end

    local s = Templates["CannonGun.CWeapon"]:GetSubClass()
    ...
    if player == Player then
        ...
        cw:ForceAnim("shot",true)
        cw._ActionState = "AltFire"
end
Network:RegisterMethod("CannonGun.StartAltFireFX", NCallOn.ServerAndSingleClient, NMode.Reliable, "eu")
--============================================================================
function CannonGun:OnFinishAltFire()
...
--============================================================================
function CannonGun:FinishAltFireFX(pe,ammo)
    local player = EntityToObject[pe]
    if not player then return end
...
Network:RegisterMethod("CannonGun.FinishAltFireFX", NCallOn.ServerAndSingleClient, NMode.Reliable, "eu")
--============================================================================
-- FIRE - ROCKET (Server Side)
--============================================================================
function CannonGun:Client_FirePrediction(first)    
    if first and self.ObjOwner.Ammo.Cannonball > 0 then
...
--============================================================================
function CannonGun:Fire() -- rocket
    local s = self:GetSubClass()

    if self.ObjOwner.Ammo.Cannonball > 0 and not self.ObjOwner._jammed then
        if Player then Player.ExplosiveFired = true end

        -- create rocket object
        local obj = GObjects:Add(TempObjName(),CloneTemplate("Cannonball_r.CItem"))
        -- set position
        ...
        obj.Pos:Set(x,y,z)
        obj.Rot:FromEulerZYX(1.57-fv.Y,-orientation,0)
...
--============================================================================
function CannonGun:FireSFX(pe)
    local player = EntityToObject[pe]
    local t = Templates["CannonGun.CWeapon"]
    ...
    if player then
        if not WEAPON_PREDICTION then
            if not Game.NoAmmoLoss then player.Ammo.Cannonball = player.Ammo.Cannonball -1 end            
        end        
        ...
        if fx and player == Player  then --and (prediction or cw.ShotTimeOut > s.FireTimeout)  then
            ...
            -- dym z lufy
            if not cw._smokepfx1 then
                if Cfg.ViewWeaponModel then
                    cw._smokepfx1 = AddPFX("hainrl_fx",0.5)
                    PARTICLE.SetImmortal(cw._smokepfx1)
                    cw._smokepfx2 = AddPFX("hainrl_fx",0.5)
                    PARTICLE.SetImmortal(cw._smokepfx2)
                    cw._smokepfx3 = AddPFX("hainrl_fx",0.5)
                    PARTICLE.SetImmortal(cw._smokepfx3)
                    cw:OnClientTick()
                end
            else
                PARTICLE.Restart(cw._smokepfx1)
                PARTICLE.Restart(cw._smokepfx2)
                PARTICLE.Restart(cw._smokepfx3)
            end
            -- blysk i swiatlo
            cw:MuzzleFlashFX(1,0.4)
            cw:ForceAnim("launch",false)        
        end
    else
...
Network:RegisterMethod("CannonGun.FireSFX", NCallOn.ServerAndAllClients, NMode.Reliable, "e")
--============================================================================
function CannonGun:OnFinishAnim(anim)
    if anim == "shot" then
        --Game:Print("endshot")
...
function CannonGun:OnUpdate()
	...
	if Game.GMode == GModes.SingleGame and self._lastfirestart + 50 < Game.currentTime  and Game.lastchat and Game.lastchat + Game.chatdelay < Game.currentTime then
		Game.lastchat=Game.currentTime
		PlaySound2D("belial/Belial_ingame_"..math.random(135,136),Game.chatvolume,nil,true)
	end
...
--MODIFIED=end##########################################################		
        ...
        self.ObjOwner.State = 31
    else
        self.ObjOwner.State = 3
    end
    --Game:Print(self._ActionState)
end
--============================================================================
function CannonGun:OnClientTick(delta)
    if self._smokepfx1 then
        local j = MDL.GetJointIndex(self._Entity,"roll_1_joint")
        local x,y,z,rw,rx,ry,rz = MDL.TransformPointByJoint(self._Entity,j,0,0,0)--0,0,0)
        ENTITY.SetPosition(self._smokepfx1,x,y,z)
        local j = MDL.GetJointIndex(self._Entity,"roll_2_joint")
        local x,y,z,rw,rx,ry,rz = MDL.TransformPointByJoint(self._Entity,j,0,0,0)--0,0,0)
        ENTITY.SetPosition(self._smokepfx2,x,y,z)
        local j = MDL.GetJointIndex(self._Entity,"roll_3_joint")
        local x,y,z,rw,rx,ry,rz = MDL.TransformPointByJoint(self._Entity,j,0,0,0)--0,0,0)
        ENTITY.SetPosition(self._smokepfx3,x,y,z)
    end
...
--============================================================================
function CannonGun:HitWallSFX(entity,x,y,z,nx,ny,nz,fx,fy,fz)
...
--============================================================================
function CannonGun:HitWaterSFX(x,y,z,nx,ny,nz,fx,fy,fz,e)
...
--    ENTITY.SpawnDecal(e,'splash',x,y,z,nx,ny,nz)
    ...
    Templates["CannonGun.CWeapon"]:Snd3D("bullet_hit_water",x,y,z)
end
--============================================================================
function CannonGun:OutOfAmmoFX(entity,fire)
    Templates["CannonGun.CWeapon"]:SndEnt("out_of_ammo",entity)
end
Network:RegisterMethod("CannonGun.OutOfAmmoFX", NCallOn.AllClients, NMode.Reliable, "eb")
--============================================================================
function CannonGun:Render()
