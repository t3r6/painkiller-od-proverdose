...
function Arrow:OnCreateEntity()    
    ...
    ENTITY.SetSynchroString(self._Entity,"Arrow.CItem"..":"..param)
    
    ENTITY.PO_SetMissile(self._Entity)
end
--============================================================================
function Arrow:OnPrecache()
...
-- =======================================================================================
function Arrow:Client_OnCreateEntity(entity,param)    
...
Arrow.vlen = 1.2
Arrow.ftime = 0.3
function Arrow:Trace(ex,ey,ez,sx,sy,sz)
...
function Arrow:Tick(delta)
    ...
    if not self.combo then
        local s = Templates["MagicCrossbow.CWeapon"]:GetSubClass()
    ...
    if e then
        ...
        if self.combo then                        
            local s = Templates["MagicArrow.CWeapon"]:GetSubClass()
        ...
        if obj and not self.r_BindedActor then
            ...
            if (obj._Class == "CActor" or obj._Class == "CPlayer") and obj._died then
                ...
                if obj._dontPinStake then
					RawCallMethod(CItem.DestroyItemFX,self._Entity,"Arrow.CItem",0)
                    GObjects:ToKill(self)
                    return
				end
...
-- =======================================================================================
function Arrow:OnHitSomething(se,e,mode)
    ...
    if mode == 1 then
        -- roztrzaskal sie
        RawCallMethod(CItem.DestroyItemFX,se,"Arrow.CItem",0)
    else        
...
Network:RegisterMethod("Arrow.OnHitSomething", NCallOn.AllClients, NMode.Reliable, "eeb") 
-- =======================================================================================
function Arrow:EditRender()
