...
function Cube:Tick(delta)
    ...
    if self._back then -- powrot
        ...
        if d <= 20 and not self._bsnd then
            Templates["RazorCube.CWeapon"]:Snd2D("head_back")
            self._bsnd = true
        end
...
--            ENTITY.SpawnDecal(e,'splash',tx,ty,tz,nx,ny,nz)
            Templates["RazorCube.CWeapon"]:Snd3D("head_hit_water",tx,ty,tz)
            self._back = true
            return
        end
        ...
        if not self._spinning then
            ...
            if obj and (obj._Class == "CActor" or obj._Class == "CPlayer") and ENTITY.GetType(e) == ETypes.Model then --and  MDL.IsRagdoll(self._Entity) then                        
                ...
                Templates["RazorCube.CWeapon"]:Snd3D("head_hit_enemy",tx,ty,tz)
            ...
            if e and not (obj and (obj._Class == "CActor" or obj.DestroyPack)) then
                ...
                ENTITY.PO_Remove(self._Entity)
                self._back = true
                MDL.SetAnim(self._Entity,"attack2",true)
            end        
        end
    end
end

function Cube:__Render()
    self.Rot:FromEntity(self._Entity) 
    local x,y,z = ENTITY.GetPosition(self._Entity)
    local x1,y1,z1 = self.Rot:TransformVector(0,0,0.5)
...
function Cube:CL_HitWallSFX(e)
    Templates["RazorCube.CWeapon"]:SndEnt("head_hit_wall",e)
end
Network:RegisterMethod("Cube.CL_HitWallSFX", NCallOn.AllClients, NMode.Unreliable, "e") 
