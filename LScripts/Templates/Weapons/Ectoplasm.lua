-- =======================================================================================
-- =======================================================================================
function Ectoplasm:OnInitTemplate()
    self:ReplaceFunction("_Synchronize","Synchronize")
    self:ReplaceFunction("Synchronize","nil")
    self.BindedActorIndex = 0
end
-- =======================================================================================
function Ectoplasm:Update()
...
-- =======================================================================================
function Ectoplasm:OnCreateEntity()    
    
    ENTITY.EnableNetworkSynchronization(self._Entity,true,false,2)
    ENTITY.SetSynchroString(self._Entity,"Ectoplasm.CItem")
    ...
    if self.ObjOwner.HasWeaponModifier then
        ...
        ENTITY.PO_SetMissile(self._Entity, MPProjectileTypes.Ectoplasm )
    end
end
--============================================================================
function Ectoplasm:OnPrecache()
	Cache:PrecacheParticleFX("GrenadeSmoke")
	Cache:PrecacheParticleFX("EctoplasmHitWall")
	Cache:PrecacheParticleFX("Ectoplasmflame")        
	Cache:PrecacheItem("Grenade.CItem")     
	Cache:PrecacheItem("stake_imp.CItem")         
	Cache:PrecacheDecal('Ectoplasm')         
    Cache:PrecacheTrail("trail_kolek_combo")
    Cache:PrecacheSounds("actor/hellangel/hella-throw-molotov")
    Cache:PrecacheSounds("weapons/Ectoplasm/Ectoplasm_onfly-loop")
    Cache:PrecacheSounds("impacts/barrel-wood-fire-loop")
end
-- =======================================================================================
function Ectoplasm:Client_OnCreateEntity(entity)    
    -- trail
    local t = ENTITY.Create(ETypes.Trail,"trail_kolek","trailName")
    ENTITY.SetPosition(t,0,0,-1.2)
    ENTITY.AttachTrailToBones(entity,t)
    local smokefx = AddPFX("Ectoplasm_shot",1)
    ENTITY.RegisterChild(entity,smokefx)
    WORLD.AddEntity(t)
    BindSoundToEntity(entity,"weapons/Ectoplasm/Ectoplasm_onfly-loop",3,18,true,nil,nil,nil,nil,0.1)
end
-- =======================================================================================
Ectoplasm.vlen = 1
Ectoplasm.ftime = 0.3
function Ectoplasm:Trace(ex,ey,ez,sx,sy,sz)
		            		...
		            		for i,o in Actors do
			                	if not o._died and o.Health > 0 then
			                    		...
			                    		local dist = Dist3D(x,y,z,self.Pos.X, self.Pos.Y, self.Pos.Z)
			                        		...
			                        		Game:Print(o._Name)
			                         		local p = Templates["GEctoObject.CProcess"]:New(o,HPDrain) 
			                          		p:Init()
...
--====================================================================
    ...
    if e then
        ...
        if self.combo then            
            ...
            grenade:Explode()
            Templates["Ectoplasm.CItem"].OnHitSomething(self._Entity,e,1)            
            GObjects:ToKill(self)
            return
        end
        ...
        if obj then
            ...
            if  (obj._Class == "CActor" or obj._Class == "CPlayer") and not obj.EctoplasmCanHitNotLinkedJoint then
            ...
            if obj.OnDamage then
                obj:OnDamage(d,self.ObjOwner,AttackTypes.Ectoplasm,tx,ty,tz,nx,ny,nz,he)  
            end
            ...
            if (obj._Class == "CActor" or obj._Class == "CPlayer") and obj._died then
                ...
                if obj._dontPinEctoplasm then
					RawCallMethod(CItem.DestroyItemFX,self._Entity,"Ectoplasm.CItem",0)
                    GObjects:ToKill(self)
                    return
				end
                ...
                if not obj.Pinned and not obj._pinnedByAI then
					...
					PlaySound3D('impacts/Ectoplasm-body'..math.random(1,2),tx,ty,tz,9,33)
                    --Game:Print(dx..", "..dy..", "..dz)
        ...
        if self.r_BindedActor then
			...
			PlaySound3D('weapons/Ectoplasm/Ectoplasm_hit'..math.random(1,2),tx,ty,tz,12)            
        end
    ...
    if obj and (obj._Class == "CActor" or obj._Class == "CPlayer") and not obj._died then
	 ...
	 mode = 1
        ...
        Templates["Ectoplasm.CItem"].OnHitSomething(self._Entity,e,mode)                                        
...
-- =======================================================================================
function Ectoplasm:OnHitSomething(se,e,mode)
    if not se then return end
        
    --Game:Print("Ectoplasm ON HIT SOMETHING")
    --ENTITY.SetPosition(se,x,y,z)                    
    
    if mode == 1 then
        -- roztrzaskal sie
        RawCallMethod(CItem.DestroyItemFX,se,"Ectoplasm.CItem",0)
    else        
        
        ENTITY.UnregisterAllChildren(se,ETypes.Trail)        
        ENTITY.KillAllChildrenByName(se,"weapons/Ectoplasm/Ectoplasm_onfly-loop")
        if ENTITY.KillAllChildrenByName(se,"Ectoplasmflame") then -- burning?
            -- add smoke
    ...
    if e then
		if ENTITY.IsFixedMesh(e) then
			...
			PlaySound3D('impacts/Ectoplasm-default'..math.random(1,5),x,y,z,12,36,false)
			...
			if b then 
				...
				if e and mode == 0 then ENTITY.SpawnDecal(e,'Ectoplasm',tx,ty,tz,nx,ny,nz) end            
				...
				AddPFX("EctoplasmHitWall",0.25,Vector:New(px,py,pz),r)
			end
		else
			...
			if obj and obj.Health and (obj.Health > 0 or obj._enabledRD or obj._gibbed)then
				if obj._Class == "CActor" and obj._enabledRD then
					--Game:Print("Ectoplasm ON HIT SOMETHING:body")
--					PlaySound3D('impacts/Ectoplasm-body'..math.random(1,2),x,y,z,12,36,false)
				elseif obj.s_SubClass and obj.s_SubClass.SoundsDefinitions and obj.s_SubClass.SoundsDefinitions.SoundHitByBullet then
					if obj.s_SubClass.SoundsDefinitions.SoundHitByBullet.samples[1] == "bullet-metal" then
						--Game:Print("Ectoplasm ON HIT SOMETHING:metal")
	--					PlaySound3D('impacts/Ectoplasm-shield'..math.random(1,3),x,y,z,12,36,false)
					else
					...
					--Game:Print("Ectoplasm ON HIT SOMETHING:rest")
				end
			end
		end
    end
...
Network:RegisterMethod("Ectoplasm.OnHitSomething", NCallOn.AllClients, NMode.Reliable, "eeb") 
-- =======================================================================================
function Ectoplasm:BurnFX(se)
    local x,y,z = ENTITY.GetPosition(se)
    local fxFire = AddPFX("Ectoplasmflame",0.25)
...
Network:RegisterMethod("Ectoplasm.BurnFX", NCallOn.AllClients, NMode.Reliable, "e") 
-- =======================================================================================
function Ectoplasm:_EditRender()
...
-- =======================================================================================
--
function Ectoplasm:OnCollision(x,y,z,nx,ny,nz,e,he)
self.HPDrainDistance = 10
end
