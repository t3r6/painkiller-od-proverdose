function Deadmeat:OnPrecache()
    Cache:PrecacheItem("VampBlood.CItem")    
end

function Deadmeat:OnCreateEntity()
 self:PO_Create(BodyTypes.FromMesh,nil,ECollisionGroups.Noncolliding)    
end

function Deadmeat:OnCollision(x,y,z,nx,ny,nz,e,he)
	--[[if e then
		...
		if self._disableDamage then
			Game:Print(self._Name.." Deadmeat col e DD "..obj)
		else
			Game:Print(self._Name.." Deadmeat col e "..obj)
		end
	else
		if self._disableDamage then
			Game:Print(self._Name.." Deadmeat col no e DD ")
		else
			Game:Print(self._Name.." Deadmeat col no e ")
		end
	end--]]
