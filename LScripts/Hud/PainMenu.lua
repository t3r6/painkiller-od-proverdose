...
--	fontBig		= "tahomabd",
--	fontSmall		= "tahomabd",
	fontBig		= "timesbd",--PrimeviL
	fontSmall	= "timesbd",-- PrimeviL
	fontBigSize	= 54,
	fontSmallSize = 26,
	fontBigTex  = "",
	fontSmallTex = "",
...
--	bgType		= MenuBackgroundTypes.Movie,
	...
	weapons = { _11 = TXT.Weapons.Razor, _12 = TXT.Weapons.Cube, _21 = TXT.Weapons.Bonegun, _22 = TXT.Weapons.Petrify, _31 = TXT.Weapons.Cannongun,
				_32 = TXT.Weapons.Cannon, _41 = TXT.Weapons.Crossbow, _42 = TXT.Weapons.Heads, _51 = TXT.Weapons.Evileye, _52 = TXT.Weapons.Scream, _61 = TXT.Weapons.Skulls, _62 = TXT.Weapons.Blade, _71 = TXT.Weapons.Ectoplasm, _72 = TXT.Weapons.GreenGoo, _81 = TXT.Weapons.Eggbomb, _82 = TXT.Weapons.EggThrow },
...
function PainMenu:Draw()
	if self.showStartMovies then
		if IsFinalBuild() then
			--PMENU.PlayMovie('../Data/Movies/LogoDreamCatcher.bik');	--
			--PMENU.PlayMovie('../Data/Movies/LogoMindware.bik');		-- PrimeviL
			--PMENU.PlayMovie('../Data/Movies/LogoNVidia.bik');		--
		end
...
--	end
	...
	PMENU.ShowMouse()
	if screen == DemoEnd or screen == DemoEnd2 then
...
--		if IsDemoMulti() or IsDemoPress() then PMENU.DisableItem( "PublicServer" ) end
	end
end
...
--	end
	...
	if IsDemoMulti() then
		if itemName == "SignAPact" or itemName == "LoadGame" then
			o.disabled = true
	...
	if IsDemoMulti() or IsDemoPress() then
		if itemName == "Quit" then
			o.action = "PainMenu:AskYesNo( Languages.Texts[469], 'PainMenu:ActivateScreen(DemoEnd)', 'PainMenu:ActivateScreen(MainMenu)' )"
		end
	end

	if itemName == "ModelSelect" and not IsDemoMulti() and not IsDemoPress() then
	...
	if skip == false then
		...
		elseif o.type == MenuItemTypes.TextButtonEx then
			...
			for i, v in o.values do
				if itemName ~= "ModelSelect" or i <= table.getn(o.values)  or IsDemoMulti() or IsDemoPress() then
		...
		elseif o.type == MenuItemTypes.TextEdit then
			...
			local value = ""
			if string.find( itemName, "AutoGenerated_" ) then
...
function SortSaveGamesByLevel(a,b)
	o = {}
	DoFile(GetSaveDirectory()..""..a.."/SaveGame.Info",false)
	if not o.Timestamp then return false end
	local levelA = o.Level
	local timeA = o.Timestamp
	o = {}
	DoFile(GetSaveDirectory()..""..b.."/SaveGame.Info",false)
	if not o.Timestamp then return false end
...
--	PMENU.AddSaveGameToList( name, "header", TXT.Menu.Level, TXT.SPStats.Playtime, TXT.Menu.SaveTime, TXT.SPStats.Difficulty )
	...
	local dirs = FS.FindFiles(GetSaveDirectory().."*",0,1)
	...
	for i=1,table.getn(dirs) do
		o = {}
		DoFile(GetSaveDirectory()..""..dirs[i].."/SaveGame.Info",false)
...
function PainMenu:AddMapTable( name, item )
	...
	self.mapsOnServerSRV = {}
	for i=1,table.getn(Cfg.ServerMapsSRV) do
		self.mapsOnServerSRV[i] = Cfg.ServerMapsSRV[i]
	end
...
function PainMenu:ApplyAudioSettings(all)
	...
	SOUND.ApplySoundSettings( master, 100, sfx, Cfg.SoundChannelsConfiguration, Cfg.SoundPan, Cfg.ReverseStereo )
	Cfg.SoundChannelsConfiguration = SOUND.GetCurrentChannelConfigurationName()
	if all then
...
function PainMenu:FixFireSwitchTable()
	if Cfg.SwitchFire[1] then
		PMENU.SetItemText( "Primary1", TXT.Weapons.Razor )
		PMENU.SetItemText( "Secondary1", TXT.Weapons.Cube )
	else
		PMENU.SetItemText( "Primary1", TXT.Weapons.Cube )
		PMENU.SetItemText( "Secondary1", TXT.Weapons.Razor )
	end

	if Cfg.SwitchFire[2] then
		PMENU.SetItemText( "Primary2", TXT.Weapons.Petrify )
		PMENU.SetItemText( "Secondary2", TXT.Weapons.Bonegun )
	else
		PMENU.SetItemText( "Primary2", TXT.Weapons.Bonegun )
		PMENU.SetItemText( "Secondary2", TXT.Weapons.Petrify )
	end

	if Cfg.SwitchFire[3] then
		PMENU.SetItemText( "Primary3", TXT.Weapons.Cannongun )
		PMENU.SetItemText( "Secondary3", TXT.Weapons.Cannon )
	else
		PMENU.SetItemText( "Primary3", TXT.Weapons.Cannon )
		PMENU.SetItemText( "Secondary3", TXT.Weapons.Cannongun )
	end
	
	if Cfg.SwitchFire[4] then
		PMENU.SetItemText( "Primary4", TXT.Weapons.Crossbow )
		PMENU.SetItemText( "Secondary4", TXT.Weapons.Heads )
	else
		PMENU.SetItemText( "Primary4", TXT.Weapons.Heads )
		PMENU.SetItemText( "Secondary4", TXT.Weapons.Crossbow )
	end

	if Cfg.SwitchFire[5] then
		PMENU.SetItemText( "Primary5", TXT.Weapons.Scream )
		PMENU.SetItemText( "Secondary5", TXT.Weapons.Evileye )
	else
		PMENU.SetItemText( "Primary5", TXT.Weapons.Evileye )
		PMENU.SetItemText( "Secondary5", TXT.Weapons.Scream )
	end

	if Cfg.SwitchFire[6] then
		PMENU.SetItemText( "Primary6", TXT.Weapons.Skulls )
		PMENU.SetItemText( "Secondary6", TXT.Weapons.Blade )
	else
		PMENU.SetItemText( "Primary6", TXT.Weapons.Blade )
		PMENU.SetItemText( "Secondary6", TXT.Weapons.Skulls )
	end
	
	if Cfg.SwitchFire[7] then
		PMENU.SetItemText( "Primary7", TXT.Weapons.Ectoplasm )
		PMENU.SetItemText( "Secondary7", TXT.Weapons.GreenGoo )
	else
		PMENU.SetItemText( "Primary7", TXT.Weapons.GreenGoo )
		PMENU.SetItemText( "Secondary7", TXT.Weapons.Ectoplasm )
	end
	if Cfg.SwitchFire[8] then
		PMENU.SetItemText( "Primary8", TXT.Weapons.Eggbomb )
		PMENU.SetItemText( "Secondary8", TXT.Weapons.EggThrow )
	else
		PMENU.SetItemText( "Primary8", TXT.Weapons.EggThrow )
		PMENU.SetItemText( "Secondary8", TXT.Weapons.Eggbomb )
	end
end
...
--      Remember that we did not requested the password from user.

        Game.PasswordAlreadyRequested = false ;
...
function PainMenu:UpdateMapTable(name,mode)
	...
	elseif self.lastMPMode == "Last Man Standing" then
		self.mapsOnServerLMS = {}
		Cfg.ServerMapsLMS = {}
	elseif self.lastMPMode == "SoulReaver" then
		self.mapsOnServerSRV = {}
		Cfg.ServerMapsSRV = {}
	end
    ...
    for i=1,table.getn(tmp_tab) do
		...
		elseif self.lastMPMode == "SoulReaver" then
			self.mapsOnServerSRV[i] = val
			Cfg.ServerMapsSRV[i] = val
		end
    end
...
--	end
	...
	elseif mode == "SoulReaver" then
		tmp_tab = self.mapsOnServerSRV
	end
...
--              If we did not ask the user for password yet, do it now.

                if not Game.PasswordAlreadyRequested then
                    Game.PasswordAlreadyRequested = true
                    PainMenu:AskForPassword( TXT.Multiplayer.EnterPasswordRequester, "PainMenu:ReJoinServer()", "PainMenu:BackToLastScreen()" )
                else

--                  The password we got was incorrect, show error and return to the browser.

                    Game.PasswordAlreadyRequested = false
		    PainMenu:ShowInfo( TXT.Multiplayer.PasswordIncorrect, "PainMenu:BackToLastScreen()" )
                end
	end
end
...
--	MDL.SetMeshLighting( self.playerModel, "*", true )
    ...
    -- cube
    MDL.SetMeshVisibility(e,"razorCube_Shape",false)
    --bonegun
   MDL.SetMeshVisibility(e,"bonegun_Shape",false)
    -- cannongun
--    MDL.SetMeshVisibility(e,"cannongun_Shape",false)
    -- cube
    MDL.SetMeshVisibility(e,"crossbow_Shape",false)
    -- screamer
    MDL.SetMeshVisibility(e,"screamer_Shape",false)
	-- hellblade
    MDL.SetMeshVisibility(e,"hellBlade_Shape",false)
    -- ecto
	MDL.SetMeshVisibility(e,"ectoplasmer_Shape",false)
	--eggs
	MDL.SetMeshVisibility(e,"eggbomb_Shape",false)
	MDL.SetMeshVisibility(e,"jesterka_Shape",false) 
...
function PainMenu_PrintGameVersion()
	...
	if PainMenu.currScreen == DemoEnd2 then return end
	local w,h = R3D.ScreenSize()
	local version = GetEXEBuild()
    HUD.SetFont("timesbd",16)
    local ver = "PROverdose: "..PROD_VERSION.." + Version: "..GetEXEBuild().."."..PK_VERSION -- CHG: PrimeviL (OD)
    if IsDemoMulti() then
    	 ver = "MP demo: "..GetEXEBuild().."."..PK_VERSION -- CHG: PrimeviL (OD)
    end
    if IsDemoPress() then
    	 ver = "Press demo: "..GetEXEBuild().."."..PK_VERSION -- CHG: PrimeviL (OD)
    end
    local tw = HUD.GetTextWidth(ver)
    HUD.PrintXY((w-tw)/2+1,40*h/768,ver,"timesbd",0,0,0,18) -- CHG:  PrimeviL (position 54+size16)
    HUD.PrintXY((w-tw)/2,39*h/768,ver,"timesbd",255,255,255,18) -- CHG:  PrimeviL (position 53+size16)
end

function PainMenu:SignAPact(mode,addon)
	if true then
		return PainMenu:SignAPactOD(mode)
	end
end

function PainMenu:SignAPactOD(mode)
	...
	if not mode or mode == 1 then
		if Game.GameInProgress then
			PainMenu:AskYesNo( Languages.Texts[686], 'Game.GameInProgress = false; Game.LevelStarted = false; Game:ClearLevelsStats(false); PainMenu:SignAPactOD(2)', 'PainMenu:ActivateScreen(MainMenu)' )
		else
			PainMenu:SignAPactOD(2)
		end
	else
		...
		if ask then
			PainMenu:AskYesNo( Languages.Texts[636], "Game.GameInProgress = false; Game.LevelStarted = false; Game:ClearLevelsStats(true); PainMenu:ActivateScreen(NewGameMenu)", "Game.GameInProgress = false; Game.LevelStarted = false; PainMenu:ActivateScreen(NewGameMenu)" )
		else
			Game.GameInProgress = false
			Game.LevelStarted = false
			PainMenu:ActivateScreen(NewGameMenu)
...
function PainMenu:SelectDifficulty(mode)
	...
	PMENU.PlayMovie("../Data/Movies/Story1.bik",self.movSndTrack)
...
function PainMenu:weaponsicons()
	...
	if (Player and Player._Entity) or spect == true then
		local item = self.currScreen.items.GeneralTab.items["weaponsicons"]
		if item.values[item.currValue] ~= Cfg.PROD_Weapons_Icons then
			Cfg.PROD_Weapons_Icons = item.values[item.currValue]
			--PainMenu:ShowInfo( "Need reconnection.", "PainMenu:ActivateScreen(ProOptions)" )
			if self.lastScreen == "server" then
				actionc = 'PainMenu:ActivateScreen(CreateServerMenu)'
			else
				actionc = 'PainMenu:ReJoinServer()'				
			end
			PainMenu:AskYesNo( "Need reconnection.", actionc, 'PMENU.ResumeSounds(); PMENU.ReturnToGame(); PainMenu:ReloadBrightskins()' )
		end
	end
end
