...
function Console:Cmd_GAMEMODE(mode)
	...
	if IsDemoPress() or IsDemoMulti() then
...
function Console:Cmd_SERVER(name)
    ...
    if name == nil then 
	    ...
	    local port = GetDefaultServerPort()
        if PMENU.StartServer( Cfg.PlayerName, "", name, 1, port ) then
            NET.LoadMapOnServer(name)
        end
    end
...
function Console:Cmd_SHOWFPS(show)
	...
	Hud._showFPS = true 
	Cfg.ShowFPS = true
	Cfg.PROD_ShowFPS = true
	end
	if show == 0 then 
	Hud._showFPS = false 
	Cfg.ShowFPS = false
	Cfg.PROD_ShowFPS = false
	end
end
--=======================================================================
